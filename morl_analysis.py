...
from MORL_analysis_utils import (
...
for num_episodes, weights, seed in itertools.product(num_episodes_list, weights_list, seeds):
    ...
    reward_matrix, actions, total_steps, params, donothing_reward_matrix, donothing_total_steps = load_saved_data(
        weights=weights,
...
for seed in seeds:
    reward_matrix_1_0_0.append(loaded_data[f"reward_matrix_episodes_{num_episodes}_weights_1_0_0_seed_{seed}"])
    reward_matrix_0_1_0.append(loaded_data[f"reward_matrix_episodes_{num_episodes}_weights_0_1_0_seed_{seed}"])
    reward_matrix_0_0_1.append(loaded_data[f"reward_matrix_episodes_{num_episodes}_weights_0_0_1_seed_{seed}"])
    reward_matrix_DoNothing.append(loaded_data[f"donothing_reward_matrix_episodes_{num_episodes}_weights_1_0_0_seed_{seed}"])
    
    total_steps_1_0_0.append(loaded_data[f"total_steps_episodes_{num_episodes}_weights_1_0_0_seed_{seed}"])
    total_steps_0_1_0.append(loaded_data[f"total_steps_episodes_{num_episodes}_weights_0_1_0_seed_{seed}"])
    total_steps_0_0_1.append(loaded_data[f"total_steps_episodes_{num_episodes}_weights_0_0_1_seed_{seed}"])
    total_steps_DoNothing.append(loaded_data[f"donothing_total_steps_episodes_{num_episodes}_weights_1_0_0_seed_{seed}"])
