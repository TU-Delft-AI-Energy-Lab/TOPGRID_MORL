...
from topgrid_morl.envs.GridRewards import TopoActionReward, LinesCapacityReward, DistanceReward, MaxDistanceReward
from topgrid_morl.envs.CustomGymEnv import CustomGymEnv  # Import your custom environment if necessary
from typing import List, Tuple

def setup_environment(env_name: str = "l2rpn_case14_sandbox", test: bool = False, action_space: int = 219, seed: int = 0, 
                      frist_reward: L2RPNReward = L2RPNReward, rewards_list: List[str] = ["LinesCapacity", "TopoAction"]) -> Tuple[GymEnv, Tuple[int], int, int]:
    """
    ...
    Returns:
        Tuple[GymEnv, Tuple[int], int, int]: Gym-compatible environment instance, observation space shape, action space dimension, and reward dimension.
    """
    ...
    # Action space setup
    gym_env.action_space = DiscreteActSpace(env.action_space, attr_to_keep=["set_bus", "set_line_status"])
