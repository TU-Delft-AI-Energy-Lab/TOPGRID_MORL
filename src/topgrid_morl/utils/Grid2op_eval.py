...
def load_action_space(env_name: str, g2op_env) -> DiscreteActSpace:
    ...
    with open(path, "rt", encoding="utf-8") as action_set_file:
        all_actions = [g2op_env.action_space(action_dict) for action_dict in json.load(action_set_file)]
...
def setup_gym_env(g2op_env_val, rewards_list: List[str], obs_tennet: List[str]) -> CustomGymEnv:
    gym_env = CustomGymEnv(g2op_env_val, safe_max_rho=0.95)
    gym_env.set_rewards(rewards_list=rewards_list)
    gym_env.observation_space = BoxGymObsSpace(g2op_env_val.observation_space, attr_to_keep=obs_tennet)
    return gym_env
