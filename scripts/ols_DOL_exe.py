...
def main(seed: int, config: str, learning_rate: float, vf_coef: float, ent_coef: float, clip_coef: float, epoch_updates: int) -> None:
    """
    ...
    agent_params["clip_coef"] = clip_coef
    agent_params["update_epochs"] = epoch_updates
    ...
    while not ols.ended() and i < 1:
        ...
        print(f"This weight will be given to the MOPPO: {w}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="base_config.json", help="Path to the configuration file")

    # Add arguments for the hyperparameters to be searched over
    parser.add_argument("--learning_rate", type=float, default=5e-5, help="Learning rate for the agent")
    parser.add_argument("--vf_coef", type=float, default=1.0, help="Value function coefficient")
    parser.add_argument("--ent_coef", type=float, default=0.02, help="Entropy coefficient")
    parser.add_argument("--clip_coef", type=float, default=0.3, help="Clip coefficient")
    parser.add_argument("--epoch_updates", type=int, default=15, help="Number of epochs for PPO updates")

    args = parser.parse_args()
    main(args.seed, args.config, args.learning_rate, args.vf_coef, args.ent_coef, args.clip_coef, args.epoch_updates)
